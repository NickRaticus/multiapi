                   ...
                   for t, p in enumerate(usergames["response"]["games"]):
                    if appid:
                      if p["appid"] == appid:
                         ...
                         try:
                          for kh , hg in enumerate(all_ach["achievementpercentages"]["achievements"]):
                              hgnames += f"{hg['name']} (not achieved) {math.trunc(hg['percent'])}% "
                             
                        
                          for kj, lp in enumerate(game_progress["playerstats"]["achievements"]):
                              ach = ach+1
                              if lp["name"] == hg["name"]:
                                 ach_ach += f"{lp['name']} (achieved) {math.trunc(hg['percent'])}%\n"
